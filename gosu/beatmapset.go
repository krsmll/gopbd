...
func (c *GosuClient) GetUserBeatmapsets(userID uint, beatmapType BeatmapType, params map[string]interface{}) ([]*Beatmapset, error) {
	var ret []*Beatmapset
	requestURL := "users/" + strconv.FormatUint(uint64(userID), 10) + "/beatmapsets/" + beatmapType.String()

	if resp, err := c.DoRequest("GET", requestURL, params); err != nil {
		fmt.Println("Error in GET request for users/{user_id}/beatmapsets/{type}:", err)
		return nil, err
	} else {
		if err = json.Unmarshal(resp, &ret); err != nil {
			fmt.Println("Error in unmarshaling GET request for users/{user_id}/beatmapsets/{type}:", err)
			return nil, err
		}
	}
